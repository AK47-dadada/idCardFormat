...
class IdCardFormat {
  ...
  getProvince (idCard) {
    ...
    if (provice) {
      const proviceCode = `${provice}0000`
      return {
   ...
   */
  getCity (idCard) {
    const getProvince = this.getProvince(idCard)
    if (Object.keys(getProvince).length > 0) {
      const { proviceCode } = getProvince
   ...
   */
  getGender (idCard) {
    if (!idCard && !this.idCardNo) return {}
    const code = this.getSpecifiedCode([-2, -1], idCard)
    // 奇数为男 偶数为女
